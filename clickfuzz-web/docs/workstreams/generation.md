...
- **2026-09-03** — Manus hosted-brief architecture: `clickfuzz_web_assemble_site_prompt()` fixed to resolve ALL `*_PLACEHOLDER` tokens before `render_prompt()` (copyright, widget, forms, core all in one pass); unresolved-placeholder validation added (throws RuntimeException → marks generation failed). `clickfuzz_web_core_generation_rules()` extended with PRICE AND DURATION BINDING + BUSINESS HOURS rules. New functions: `clickfuzz_web_generation_brief_url()` and `clickfuzz_web_manus_bootstrap_message()`. Manus cron path now stores full rendered brief in `generation_prompt` then sends only a short bootstrap message to `task.create`; bootstrap references the hosted brief at `pitchsnap/generation_brief/{preview_token}`. New public endpoint `Pitchsnap_runtime::generation_brief()` serves stored `generation_prompt` as no-auth HTML; routes added to both route files. Anthropic path unchanged — still receives full inline prompt.
